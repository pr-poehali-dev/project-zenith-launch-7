...
export default function HeroSection() {
  ...
  const [profile, setProfile] = useState<Record<string, string>>({
    ...
    bio: 'Воспитатель, исследователь и немного волшебник — каждый день превращаю обычные занятия в увлекательные творческие приключения.',
    experience: '5',
          ...
          <div className="flex max-w-2xl flex-col gap-12">
            ...
            <div className={cn('transform transition-all duration-1000 delay-300 ease-out', isLoaded ? 'translate-x-0 opacity-100' : '-translate-x-8 opacity-0')}>
              <div className="space-y-4">
                <p className="text-3xl font-light text-white md:text-4xl lg:text-5xl">{profile.name}</p>
                <p className="text-xl font-light text-white/80 md:text-2xl">{profile.title}</p>
                <p className="text-base font-light text-white/60 md:text-lg max-w-lg pt-2">{profile.bio}</p>

                <div className="flex items-center gap-3 pt-1">
                  <div className="h-px w-8 bg-white/40" />
                  <p className="text-xs text-white/50 uppercase tracking-widest">{profile.experience} лет опыта</p>
                </div>

                <div className="flex gap-4 pt-2">
                  <a href="#contact" className="inline-block rounded-full bg-white px-6 py-2.5 text-sm font-medium text-black transition-opacity hover:opacity-90">
                    Написать мне
                  </a>
                </div>

                <div className="flex gap-6 pt-2">
                  <a href={profile.telegram} target="_blank" rel="noopener noreferrer" className="text-white/70 transition-colors hover:text-white" aria-label="Telegram">
                  ...
                  <a href={profile.vk} target="_blank" rel="noopener noreferrer" className="text-white/70 transition-colors hover:text-white" aria-label="VKontakte">
                    <svg className="h-7 w-7" fill="currentColor" viewBox="0 0 24 24">
                  ...
                  <a href={profile.instagram} target="_blank" rel="noopener noreferrer" className="text-white/70 transition-colors hover:text-white" aria-label="Instagram">
                    <svg className="h-7 w-7" fill="currentColor" viewBox="0 0 24 24">
